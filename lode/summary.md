...
- Auto-generated via `vite-plugin-pwa` with Workbox
- Precaches: JS, CSS, HTML, icons, banner, profile image (~1MB)
- Excludes large files (card.png) from cache
- `registerType: 'prompt'` - user chooses when to update

**Update Flow**:
1. Service worker checks for updates on each visit
2. If new version found, `onNeedRefresh()` callback fires
3. Update prompt appears (bottom-right, non-blocking)
4. User clicks "UPDATE" → `updateSW(true)` reloads with new version
5. User clicks "LATER" → dismisses prompt, asked again next visit
...
**Update Prompt UI** (#update-prompt):
- Fixed position bottom-right (away from HUD and touch controls)
- Game-matching aesthetic (blue border, green text)
- z-index 10000 (above all game elements)
- Buttons: "UPDATE" (green) and "LATER" (grey)
